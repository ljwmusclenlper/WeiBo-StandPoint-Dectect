...
def convert_single_example_simple(max_seq_length, tokenizer, text_a, text_b=None):
    ...
    #===================================================================================================
    if len(tokens_a) > 45:
        tokens_a = tokens_a[:45]
    else:
        tokens_a += (45 - len(tokens_a))*['[PAD]']
    #=============改动的地方，保证tokens_a都是45个字符，方便后面截取，token_b就是bert_pooled_out[47:]========
    tokens_b = None
    if text_b:
        tokens_b = tokenizer.tokenize(text_b)  # 这里主要是将中文分字,返回的是列表
    if tokens_b:
